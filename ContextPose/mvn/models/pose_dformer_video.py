...
class DeformableBlock(nn.Module):
    ...
    def __init__(self, feature_dim_list, dim, num_heads, num_samples, qkv_bias=False, drop_path=0., mlp_ratio=2, norm_layer=nn.LayerNorm,
                 # NEWLY ADDED ARGS BY BRADLEY 250718:
                 max_frames=16,
                 ):
        super().__init__()
        ...
        head_dim = dim // num_heads
        self.norm1 = norm_layer(dim)    # LayerNorm((128,), eps=1e-05, elementwise_affine=True)
        self.attention_weights = nn.Linear(dim, num_heads * num_samples)    # Linear(128,16)
    ...
    def forward(self, x, ref, features_list):
        # x: [B,5,T,17,128]
        # ref: [B,T,17,2]
        # features_list: [[B,T,32,64,48], [B,T,64,32,24], [B,T,128,16,12], [B,T,256,8,6]]
        x_0, x = x[:, :1], x[:, 1:] # [B,1,T,17,128], [B,4,T,17,128]
        b, l, t, p, c = x.shape    # l=4 (levels), p=17 (num_joints)
        residual = x    # [B,4,T,17,128]
        x = self.norm1(x + x_0) # [B,4,T,17,128]
...
class PoseTransformer(nn.Module):
    def __init__(self, config=None, backbone='hrnet_32', num_joints=17, in_chans=2,
        ...
        # NEWLY ADDED PARAMS BY BRADLEY 250718:
        # self.Temporal_pos_embed = nn.Parameter(torch.zeros(1, 1+self.levels, max_frames, embed_dim_ratio))    # TODO. 在线推理能否使用这个?

    def forward(self, keypoints_2d, ref, features_list):
        # keypoints_2d (keypoints_2d_cpn): [B,T,17,2]; 
        # ref (keypoints_2d_cpn_crop): [B,T,17,2]; 
        # features_list: [torch.Size([B,T,32,64,48]), torch.Size([B,T,64,32,24]), torch.Size([B,T,128,16,12]), torch.Size([B,T,256,8,6])]
        b, t, p, c = keypoints_2d.shape
        ### now x is [batch_size, 2 channels, receptive frames, joint_num], following image data
        x = self.coord_embed(keypoints_2d)  # x: [B,T,17,128]
        
        features_list_timeFlattened = [rearrange(features, 'b t c h w -> (b t) c h w') for features in features_list]  # [B*T,32,64,48], [B*T,64,32,24], [B*T,128,16,12], [B*T,256,8,6]
        ref_timeFlattened = rearrange(ref, 'b t j c -> (b t) j c')  # [B*T,17,2]
        features_ref_timeFlattened_list = [
            F.grid_sample(features, ref_timeFlattened.unsqueeze(-2), align_corners=True).squeeze(-1).permute(0, 2, 1).contiguous() \
            for features in features_list_timeFlattened]
        ...
        features_ref_list = [rearrange(features, '(b t) j c -> b t j c', t=t) for features in features_ref_timeFlattened_list]
        features_ref_list = [embed(features_ref_list[idx]) for idx, embed in enumerate(self.feat_embed)]
        # [torch.Size([B, T, 17, 128]), torch.Size([B, T, 17, 128]), torch.Size([B, T, 17, 128]), torch.Size([B, T, 17, 128])]
        x = torch.stack([x,*features_ref_list], dim=1) # [b, 5, T, 17, 128]

        x += self.Spatial_pos_embed.unsqueeze(-3)
        # x += self.Temporal_pos_embed.unsqueeze(-2)    # TODO. 在线推理能否使用这个?
        x = self.pos_drop(x)
