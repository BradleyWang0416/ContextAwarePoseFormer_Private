...
class PoseTransformer(nn.Module):
    ...
    def forward(self, keypoints_2d, ref, features_list):
        # keypoints_2d (keypoints_2d_cpn): [B,17,2]; 
        # ref (keypoints_2d_cpn_crop): [B,17,2]; 
        # features_list: [torch.Size([B,32,64,48]), torch.Size([B,64,32,24]), torch.Size([B,128,16,12]), torch.Size([B,256,8,6])]
        b, p, c = keypoints_2d.shape
        ### now x is [batch_size, 2 channels, receptive frames, joint_num], following image data
        x = self.coord_embed(keypoints_2d)  # x: [B,17,128]
        ...
        for blk in self.context_blocks: # blk: <class 'mvn.models.pose_dformer.DeformableBlock'>
